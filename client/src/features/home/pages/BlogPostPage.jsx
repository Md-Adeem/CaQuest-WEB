  ...
  if (!post) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
        ...
        <p className="text-gray-600 dark:text-gray-300 mb-8">We couldn't find the blog post you're looking for.</p>
  ...
  return (
    <article className="min-h-screen bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {/* Article Header */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-8 md:p-12 -mt-20 relative z-10 mx-4 md:mx-8">
          ...
          <div className="prose prose-lg prose-primary max-w-none text-gray-700 dark:text-gray-200">
            {post.content.split('\n\n').map((paragraph, index) => (
              paragraph.startsWith('###') ? (
                <h3 key={index} className="text-2xl font-bold text-gray-900 dark:text-white mt-10 mb-5 pb-2 border-b border-gray-100 dark:border-gray-700">
                  {paragraph.replace('### ', '')}
                </h3>
              ) : (
                <p key={index} className={`mb-6 leading-relaxed text-lg text-gray-700 dark:text-gray-200 ${index === 0 ? 'first-letter:text-7xl first-letter:font-extrabold first-letter:text-primary-600 first-letter:float-left first-letter:mr-3 first-letter:-mt-2 first-line:uppercase first-line:tracking-widest' : ''}`}>
                  {paragraph}
                </p>
              )
            ))}
          ...
          <div className="mt-16 pt-8 border-t border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Editor & CA Mentor</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400 mr-2">Share:</span>
              <button className="p-2 rounded-full bg-gray-50 dark:bg-gray-900 text-gray-600 dark:text-gray-300 hover:bg-primary-50 hover:text-primary-600 transition-colors" title="Share via Link">
                <HiOutlineLink className="w-5 h-5" />
              </button>
              <button className="p-2 rounded-full bg-gray-50 dark:bg-gray-900 text-gray-600 dark:text-gray-300 hover:bg-primary-50 hover:text-primary-600 transition-colors" title="Share Article">
      ...
      <div className="max-w-7xl mx-auto mt-24">
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-8 border-l-4 border-primary-600 pl-4">Related Articles</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {relatedPosts.map(related => (
            <div key={related.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
